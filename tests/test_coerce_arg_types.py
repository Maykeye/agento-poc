...
from llm_fix import coerce_arg_types
...
class TestCoerceArgTypes(TestBase):
    ...
    def test_coerce_string_to_integer(self):
        ...
        assert isinstance(result["num"], int), f"Expected int, got {type(result['num'])}"
    ...
    def test_coerce_string_to_float(self):
        ...
        args = {"num": "3.14"}
        result = coerce_arg_types(args, tool)
        assert isinstance(result["num"], float), f"Expected float, got {type(result['num'])}"
