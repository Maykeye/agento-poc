...
def _editor_get_current_state():
    ...
    if llm_id not in ToolEditor._current_state:
        raise ValueError("Not in editor mode")

    state = ToolEditor._current_state[llm_id]
...
def _editor_find_unique_pattern(path: str, pattern: str) -> tuple[int, int, str, int]:
    ...
    if total_matches == 0:
        raise ValueError(
            f"Pattern '{pattern}' not found in file. " f"Total occurrences: 0"
        )
...
def _editor_format_search_result(
    ...
    ToolEditor._current_state[llm_id].current_line = new_current_line
...
class EditorToolPrint(Tool):
    ...
    def __call__(self):
        ...
        state = ToolEditor._current_state[id(llm)]
...
After goto, old buffer messages are pruned to save context.""",
    ...
    def __call__(
        ...
        path = ToolEditor._current_state[llm_id].path
        ...
        # Update current line
        ToolEditor._current_state[llm_id].current_line = line_number
...
+added line
 context line""",
    ...
    def __call__(
        ...
        current_line = ToolEditor._current_state[llm_id].current_line
        path = ToolEditor._current_state[llm_id].path
        ...
        try:
            ...
            if current_line > new_total_lines:
                ...
                ToolEditor._current_state[llm_id].current_line = current_line
...
All edit commands after read_file will apply to the file that was edited before reading""",
    ...
    def __call__(self, path: Annotated[str, "Path to the file to read"]):
        ...
        if isinstance(result, str):
            llm = LLM.INSTANCES[-1].llm
            llm_id = id(llm)
            current_path = ToolEditor._current_state[llm_id].path
...
Use this when you need to edit other file from the current one quckly, but it's preferrably to finish_editing and in report state what needs to be edited.""",
    ...
    def __call__(self, path: Annotated[str, "Path to the new file to edit"]):
        ...
        old_file = ToolEditor._current_state[llm_id]
        ToolEditor._current_state[llm_id] = EditorEntry(path, 1)
...
The editor will exit and return control to the main LLM.""",
    ...
    def __call__(
        ...
        current_editing_path = ToolEditor._current_state[llm_id].path
