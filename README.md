...
* Context-mode. Currently file operation supports three kinds of operation: 
    ...
    * Prefix mode. First message from user is "file context". It contains content of files being operated on so far. If the same file read twice, the context will contains only one, latest version of the content. It often confuses qwen-35B
    * Suffix model. Rewrites old versions of the file. For example if tool read `foo.txt`, then decided to edit it, first read will be removed and in edit panel whole file will be output
